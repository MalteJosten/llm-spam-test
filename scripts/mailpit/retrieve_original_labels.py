...
def save_summary(summary, save_name):
    save_file_path = os.path.join(SAVE_PATH, f"sum_orig_{save_name}.json")
...
def start_test():
    labels = []
    save_file_path = os.path.join(SAVE_PATH, f"gt_{os.path.basename(MAIL_DIR)}.json")
    ids = []
    ...
    with open(save_file_path, "w") as gt_file:
        json.dump({"ids": ids}, gt_file, indent=2)
        print(f"  >> [{ts_time()}] Saved ground truth at {save_file_path}")

    summary = create_summary(labels)
    save_summary(summary, os.path.basename(MAIL_DIR))
